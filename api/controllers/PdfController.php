...
class PdfController {
    public function generate($id) {
        ...
        $signatureUrl = $settings['company_signature'] ?? '';
        ...
        $html .= '</div><div style="clear: both;"></div></div></body></html>';
        ...
        try {
            ...
            // Save to storage anyway for sharing/caching
            ...
            Response::json([
                'status' => 'success',
                'url' => BASE_URL . '/api/v1/pdf/' . $id . '?type=' . $type . '&token=' . ($_GET['token'] ?? '') . '&action=view'
            ]);
        } catch (Exception $e) {
